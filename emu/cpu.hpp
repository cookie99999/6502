...
public:
  ...
  int eval() {
    ...
    case 0xf5:
    case 0xe1:
    case 0xeb: //this one is undocumented but appears to work the same
    case 0xf1: //SBC
      operand ^= 0xff;
      tmp = a;
      a = a + operand + ((p & F_C) ? 1 : 0);
    ...
    case 0xbb: //LAS
      ...
      pc += instr_set[opcode].bytes;
      break;
    case 0x9f: //SHA Y-absolute
      operand = (uint8_t)((fetch_addr(pc + 1) & 0xff00) >> 8) + 1;
      tmp = a & x & operand;
      store_mem_byte(store_addr, tmp);
      pc += instr_set[opcode].bytes;
      break;
    case 0x93: //SHA Y-indirect zp
      operand = fetch_mem_byte(fetch_addr(fetch_mem_byte(pc + 1)));
      tmp = a & x & operand;
      store_mem_byte(store_addr, tmp);
      pc += instr_set[opcode].bytes;
      break;
    ...
    case 0x2b: //ANC
      a &= operand;
      p = (a == 0) ? p | F_Z : p & ~F_Z;
      p = (a & F_N) ? p | F_N | F_C : p & ~(F_N | F_C);
      pc += instr_set[opcode].bytes;
      break;
    case 0x6b: //ARR
      tmp = (a & operand) >> 1;
      tmp &= ((p & F_C) << 7) | 0b01111111;
      a = tmp;
      //todo: flags different in decimal mode
